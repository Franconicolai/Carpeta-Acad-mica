...
def resolver_problema_generacion_columnas(tamano_items, tamano_materia_prima, demanda, error):
    ...
    maestro.setObjective(sum(x[k] for k in range(K)),GRB.MINIMIZE)
    maestro.update()
    ...
    # las iteraciones.
    pi = [1]*m
    satelite.setObjective(sum(pi[i]*y[i] for i in range(m)),GRB.MAXIMIZE)
    satelite.update()
    ...
    while (iteracion < NITER) :
        ...
        if satelite.ObjVal <= 1+error: # break if no more columns
            break
...
if __name__ == '__main__':
    ...
    rollos = resolver_problema_generacion_columnas(tamano_items, tamano_materia_prima, demanda, error)
