...
P_0 = 150000 # Presión inicial del aire en Pa
...
# Definimos constantes para la ecuación de gases ideales
R = 287.05  # Constante de gas ideal para el aire en J/(kg·K)
T = 300  # Temperatura en Kelvin (aproximadamente 27°C)
rho_aire = 1.225  # Densidad del aire en kg/m^3
...
k = A_boquilla * np.sqrt(2 * R * T / rho_aire)

# Función para calcular el caudal de agua
def dV_agua_dt(V_agua):
    V_aire = V_total - V_agua
    if V_aire > 0 and V_agua > 0:
        return -k * np.sqrt((V_aire - V_agua) / (V_aire * V_agua))
    else:
        return 0
    
# Función para calcular el empuje
def calcular_fuerza_empuje(V_agua, V_aire0):
    V_aire = V_total - V_agua  # Actualizar el volumen de aire
    P_aire = P_0 * (V_aire0 / V_aire) ** gamma  # Calcular la presión del aire
    v = np.sqrt((2 * (P_aire - P_atm)) / 1000)  # Velocidad de salida
    m_dot = rho_agua * dV_agua_dt(V_agua)  # Tasa de cambio de masa
    f_empuje = m_dot * v  # Calcular el empuje
    return f_empuje
...
def runge_kutta_4(V_agua0, t0, tf, h):
    ...
    return tiempo_empuje, dq_agua_dt, masa_dt, f_empuje_list

# Simulación
tiempo_empuje, dq_agua_dt, masa_dt, f_empuje_list = runge_kutta_4(V_agua0, t0, tf, h)
